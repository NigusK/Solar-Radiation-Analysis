...
class EDA:
    ...
    def replace_nagative_values(self,df):
        df_copy = df.copy()
        df_copy[['GHI', 'DNI', 'DHI']] = df_copy[['GHI', 'DNI', 'DHI']].clip(lower=0)
        return df_copy

    def outliers_fun(self,df, cols):    
        outliers_arr = []
        threshold = 3
    ...
    def mean_median_std_comparison(self,df1,df2,df3):
        ...
        plt.show()
        ########################
    def plot_daily_mean(self,df1, df2, df3):
    ...
    def plot_over_an_hour(self,df1, df2, df3):
    ...
    def plot_over_a_day(self,df1, df2, df3):
        # Convert 'Timestamp' columns to datetime
        for df in [df1, df2, df3]:
            df['Timestamp'] = pd.to_datetime(df['Timestamp'])
    ...
    def plot_over_a_month(self,df1, df2, df3):
        # Convert 'Timestamp' columns to datetime
        for df in [df1, df2, df3]:
            df['Timestamp'] = pd.to_datetime(df['Timestamp'])
    ...
    def merge_df(self,df1,df2,df3):
        df1 = df1.reset_index() if 'Timestamp' in df1.index.names else df1
        df2 = df2.reset_index() if 'Timestamp' in df2.index.names else df2
        df3 = df3.reset_index() if 'Timestamp' in df3.index.names else df3
        dfs = [df1,df2,df3]
    ...
    def polar_plot(self,df):
        ...
        plt.show()
    def temperatur_analysis(self,df):
        ...
        plt.tight_layout()
        plt.show()
    def frequency_distribution(self,df):
